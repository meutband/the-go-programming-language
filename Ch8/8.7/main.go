// This packages saves a local mirror of a website
package main
...
func crawl(url string, depth int, wg *sync.WaitGroup) {
	...
	<-tokens
	if err != nil {
		log.Printf("extract %s: %s", url, err)
	}
...
// linkNodes captures every <a> element the node
...
// linkURLs extracts the href attributes and discards any non base host
...
// rewriteLocalLinks rewrites local links to be relative index.html files
...
// extract is similar to gopl.io/ch5/links but fetches the content
// from the extracted links so that each references can be downloaded
// and references
func extract(rawurl string) (urls []string, err error) {
	...
	if base.Host != u.Host {
		log.Printf("not saving %s: mismatched hosts", rawurl)
		return nil, nil
	}
