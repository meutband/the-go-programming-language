...
### NOTES:
To generate the data.xml from the example to use in [7.17](#717) and [7.18](#718), enter the following. 
```
> go install gopl.io/ch1/fetch@latest
> fetch http://www.w3.org/TR/2006/REC-xml11-20060816 > data.xml
```

## 7.1
Using the ideas from `ByteCounter`, implement counters for words and for lines. You will find `budio.ScanWords` useful.
...
## 7.3
Write a `String` method for the `*tree` type in `gopl.io/ch4/treesort` (&sect;4.4) that revelas the seqence of values in the tree.

## 7.4
The `strings.NewReader` function returns a value that satisfies the `io.Reader` interface (and others) by reading from its argument, a string. Implement a simple version of `NewReader` yourself, and use it to make the HTML parser (&sect;5.2) take input from a string.

## 7.5
The `LimitReader` function in the `io` pacakge accepts an `io.Reader` `r` and a number of bytes `n`, and returns another `Reader` that reads from `r` but reports an end-of-file condition after `n` bytes. Implement it.
```
func LimitReader(r io.Reader, n int64) io.Reader
```

## 7.6
Add support for Kelvin temperature to `tempflag`.

## 7.7
Explain why the help message contains &deg;C when the default value of `20.0` does not.

## 7.8
Many GUIs provide a table widget with a stateful multi-tier sort: the primary sort key is the most recently clicked column head, the secondary sort key is the second-most recently clicked column head, and so on. Define an implementation of `sort.Interface` for use by such a table. Compare that aprroach with repeated sorting using `sort.Stable`.

## 7.9
Use the `html/template` package (&sect;4.6) to replace `printTracks` with a function that displays the tracks as an HTML table. Use the solution to the previous exercise to arrange that each click on a column head makes an HTTP request to sort the table.
...
## 7.11
Add additional handlers so that clients can create, read, update, and delete dataase entries. For example, a request of the form `/update?item=socks&price=6` will update the price of an item in the inventory and report an error if the item does not exist or if the price is invalid. (Warning: this change introduces concurrent variable updates.)

## 7.12
Change the handler for `/list` to print its output as an HTML table, not text. You may find the `html/template` package (&sect;4.6) useful.
...
## 7.14
Define a new concrete type that satisfies the `Expr` interface and provides a new operation such as computing the minimum value of its operands. Since the `Parse` function does not creat instances of this new type, to use it you will need to construct a syntax tree directly (or extend the parser).
