// This package pulls the package requirments and checks to see if there is any
// matches with command arguments.
package main
...
func ancestors(packageNames []string) []string {
	...
	p := strings.Join(packageNames, " ")
	args := []string{"list", `-f={{.ImportPath}} {{join .Deps " "}}`, p}
	out, err := exec.Command("go", args...).Output()
	if err != nil {
